...
class driver():
    def __init__(self,manual_control='keyboard',run_NN=False,model="steering",model_path='./', auto_throttle=0.05):
        ...
        if(run_NN):
            if(model == "steering"):
                print("INFO message: using steering model")
                ...
                proc_model_runner.setDaemon(True)
                proc_model_runner.start()
            elif (model == "steering_dave2"):
                print("INFO message: using steering_dave2 model")
                self.NN = model_runner('steering_dave2', self.WHEELBASE, model_path=model_path)
                proc_model_runner = threading.Thread(
                    target=self.NN.run_model)  # daemon -> kills thread when main program is stopped
                proc_model_runner.setDaemon(True)
                proc_model_runner.start()
            elif(model=="steering_resnet"):
                print("INFO message: using steering_resnet model")
                self.NN = model_runner('steering_resnet', self.WHEELBASE)
                proc_model_runner = threading.Thread(target=self.NN.run_model)  # daemon -> kills thread when main program is stopped
                proc_model_runner.setDaemon(True)
                proc_model_runner.start()
            else:
    ...
    def update_state(self,X,Y,speed,steer,image_C,image_L,image_R,time_stamp):
        ...
        if(self.NN):
            ...
            self.NN.update_model_input(self.cam_img, self.speed, self.auto_throttle, self.dt,time_stamp)
