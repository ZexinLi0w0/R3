...
class ParallelEnvExperiment(Experiment):
    ...
    def train(self, frames=np.inf, episodes=np.inf):
        ...
        while not self._done(frames, episodes):
            training_begin = timer() # NEW
            action = self._agent.act(state_array)
            training_end = timer()  # NEW
            training_time_elapsed += (training_end - training_begin) # NEW
            ...
            if episodes_completed > 0:
                ...
                for i in range(num_envs):
                    if dones[i]:
                        ...
                        training_fps = 0 # (self._frame - start_frame) / training_time_elapsed * self._agent.minibatch_size

                        self._elapsed_train_time += training_time_elapsed
                        self._log_training_episode(returns[i], fps, training_fps, self._elapsed_train_time)
                        returns[i] = 0
