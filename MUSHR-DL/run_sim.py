...
class SimpleClient(SDClient):
    ...
    def __init__(self, address, args, poll_socket_sleep_time=0.001):
        ...
        self.car = driver(manual_control=args.manual_control,run_NN=args.test,model=args.model,model_path=args.model_path, auto_throttle=args.auto_throttle) # this class is defined in the file CAR.py

    def on_msg_recv(self, json_packet):
        # global car
        if json_packet['msg_type'] == "scene_names":
            print("scene_names: {}".format(json_packet['scene_names']))
        ...
        if json_packet['msg_type'] == "telemetry":
            ...
            imgString = json_packet["image"] # camera image
            image = Image.open(BytesIO(base64.b64decode(imgString)))
            image_C = np.asarray(image)
            image_C = cv2.cvtColor(image_C,cv2.COLOR_BGR2GRAY)

            print(json_packet.keys())
            # exit(0)

            if(not self.car.init):
                self.car.initialize(json_packet["gyro_x"],json_packet["gyro_z"],0,json_packet["speed"],0.5)
                # self.car.initialize(json_packet["gyro_x"],json_packet["gyro_z"],0,0.1,0.5)
                pass
            else:
                ...
                self.car.update_state(json_packet["gyro_x"],json_packet["gyro_z"],json_packet["speed"],json_packet["steering_angle"], image_C, [], [], time_stamp)
                # print("DBG: SPEED: {}".format(json_packet["throttle"]))
    ...
    def send_controls(self, steering, throttle):
        # throttle = 0.5
        # print("DBG: THROTTLE: {}".format(throttle))
        p = { "msg_type" : "control",
...
def test_clients(args):
    print("is test : {}".format(args.test))
    # test params
    host = "169.235.18.78" # local host
    port = 9091
    num_clients = 1
    clients = []
    ...
    while(not loaded):
        ...
        for c in clients:
            loaded = c.car_loaded           
    # Car config
    msg = '{ "msg_type" : "car_config", "body_style" : "mushr", "body_r" : "0", "body_g" : "0", "body_b" : "255", "car_name" : "RapidLearn", "font_size" : "80" }'
    clients[0].send(msg)
    time.sleep(1)
    ...
    while not c.car.Finish and do_drive:
        try:
            for c in clients:
                c.update()
                # print(c)
                # print("aborted: {}".format(c.aborted))
                # print("car.Finish: {}".format(c.car.Finish))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path', type=str, default="./", help='model state dictionary path')
    ...
    parser.add_argument('--auto_throttle', type=float, default=0.05, help='constant auto throttle for driving')
